...
//        subjectId = intent.getIntExtra("subject_id", 0);
    ...
    private void makeApiCall() throws JSONException {
        ...
        connectionAsync.execute(jsonObject.toString(), Config.serverAddress + "classes/classesCalendarForSubjectGroup/");
    }
    ...
    public void onPostExecute(String jsonString, int responseCode) {
        ...
        try {
            ...
            for(int i=0; i<jsonArray.length(); i++){
                ...
                //date = simpleDateFormat.format(new SimpleDateFormat("yyyy-MM-dd").parse(jsonObject.getString("lesson_date")));
                Calendar calendar = Calendar.getInstance();
                calendar.setTime(new SimpleDateFormat("yyyy-MM-dd").parse(jsonObject.getString("lesson_date")));
                //Log.v("date1:", calendar.getTime().toString());
