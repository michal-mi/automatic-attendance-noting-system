...
public class QRCode extends AppCompatActivity implements AsyncPostResponse {
    ...
    private void makeApiCall() throws JSONException {
        ...
        connectionAsync.execute(jsonObject.toString(), Config.serverAddress + APIRoutes.QR_CODE);
    }
    ...
    public void onPostExecute(String responseJsonString, int responseCode) {
        ...
        else if (responseCode == ResponseCodes.QR_CODE_WRONG_TIME){
            makeSimpleAlert("Minął czas na odnotowanie obecności." +
                    " Obecność nie została odnotowana.");
        }
        else if (responseCode == ResponseCodes.QR_CODE_WRONG_LOCATION){
            makeSimpleAlert("Niepoprawna lokalizacja. Obecność nie została odnotowana.");
        }
        else if (responseCode == ResponseCodes.QR_CODE_ALREADY_PRESENT){
