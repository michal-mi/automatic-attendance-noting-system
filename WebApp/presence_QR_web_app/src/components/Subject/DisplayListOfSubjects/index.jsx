...
const DisplayListOfSubjects = () => {
    ...
    const [subjectSemesterForSearch, setSubjectSemesterForSearch] = useState("")
    ...
    async function handleSearch(event) {
        const searchSubjects = async () => {
            try {
                var dataToSend = { id: facilityID, semester: subjectSemesterForSearch }
                var url = 'http://localhost:8080/subjects/searchSubjects/'
    ...
    useEffect(() => {
        ...
        const fetchAndSetAllSubjectsFromFacility = async () => {
            try {
        ...
        fetchAndSetAllSubjectsFromFacility()
    }, []);
                    ...
                    <div className={styles.box_border_left}>
                        <form onSubmit={handleSearch}>
                            ...
                            <div className={styles.radio_box}>
                                <select name="roles" id="roles" className={styles.radio_input} onChange={e => setSubjectSemesterForSearch(e.target.value)} required>
                            ...
                            <button type="submit"
                                className={styles.search_subject_btn}>
                                Wyszukaj
                            </button>
                        </form>
                    </div>
                    ...
                    <div className={styles.box_border_right}>
                        {
                            allSubjects.map((entry, key) => (
                                entry.id !== null ?
                                    <div className={styles.one_subject_data}>
                                        ...
                                        <button className={styles.navigation_btn}>
                                            <Link className={styles.no_underline} to={"/editSubject/" + entry.id}>
                                                Szczegóły
                                            </Link>
                                        </button>
                                    </div>
                                    :
                                    <div>Brak przedmiotów dla tej placówki!</div>
                            ))
                        }
                    </div>
                </div>
...
export default DisplayListOfSubjects